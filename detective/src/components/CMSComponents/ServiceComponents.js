...
export default memo(function ServiceComponents(props) {
  ...
  return (
    <Link to="/work/service">
      <Card
        className="bg-dark text-white service-card"
        onMouseLeave={() => setIsHover(false)}
        onMouseEnter={() => setIsHover(true)}>
        <Card.Img
          className={`service-image ${isHover ? "shadow" : ""}`}
          src={imgSrc}
          alt="Card image"
        />
        <Card.ImgOverlay>
          <Card.Title className={`card-title-text ${isHover ? "hide" : ""}`}>
            {props.data.title}
          </Card.Title>
